...
SimpleForm.setup do |config|
  ...
  config.wrappers :default, :class => :wrapper,
    ...
    b.wrapper :tag => 'div', :class => 'form-group' do |ba|
      ...
      ## Inputs
      ba.use :label_input
      ba.use :hint, :wrap_with => { :tag => :span, :class => "hint help-block" }
  ...
  config.wrappers :selectboxformat, tag: :div, class: "form-group", error_class: "has-error" do |b|
    # Form extensions
    b.use :html5
    b.use :placeholder
    b.wrapper tag: :label, :class => 'col-md-2 control-label' do |labelclass|
  ...
  config.wrappers :checkboxformat, tag: :div, class: "checkbox" do |b|
    # Form extensions
    b.use :html5
  ...
  config.wrappers :textboxformat, tag: :div, class: "form-group", error_class: "has-error" do |b|
    # Form extensions
    b.use :html5
    b.use :placeholder
    b.wrapper tag: :label, :class => 'col-md-2 control-label' do |labelclass|
  ...
  %w(StringInput RangeInput PasswordInput TextInput NumericInput).each do |class_name|
  ...
  config.wrapper_mappings = { :string => :textboxformat,
                              :text => :textboxformat,
                              :range => :textboxformat,
                              :password => :textboxformat,
                              :numeric => :textboxformat,
                              :boolean => :checkboxformat,
                              :url => :textboxformat,
                              :file => :textboxformat,
                              :email => :textboxformat,
                              :tel => :textboxformat,
                              :float => :textboxformat,
                              :integer => :textboxformat,
                              :decimal => :textboxformat,
                              :select => :selectboxformat,
                              :grouped_select => :selectboxformat,
                              :image_preview => :textboxformat,
